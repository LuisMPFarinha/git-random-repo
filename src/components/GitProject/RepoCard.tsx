...
const RepoCard = ({ repo }: { repo: any }) => {
  return (
    <div className="relative flex flex-col w-full h-full bg-white border-2 rounded-2xl py-2 px-3 gap-1">
      <label className="font-semibold">{repo.data?.name}</label>
      <label className="text-gray-500 text-sm mb-7">{repo.data?.description}</label>
